...
def get_job_recommendations(request):
    ...
    try:
        ...
        for job_data, job_instance in zip(
            serializer.data, recommended_jobs, strict=False
        ):
            recommendation_info = {
                **job_data,
                "recommendation_score": getattr(
                    job_instance, "recommendation_score", 0.5
                ),
                ...
                # Indica si es fallback por falta de embedding
                "is_fallback": not user_embedding,
            }
        ...
        return Response(
            {
                ...
                "recommendation_type": (
                    "vectorial" if user_embedding else "trending_fallback"
                ),
            }
        )
...
def get_trending_jobs(request):
    ...
    try:
        ...
        trending_data = []
        for job_data, job_instance in zip(serializer.data, trending_jobs, strict=False):
            trending_info = {
                **job_data,
        ...
        return Response(
            {"trending_jobs": trending_data, "total_count": len(trending_data)}
        )
...
def track_job_interaction(request, job_id):
    ...
    try:
        ...
        try:
            Jobs.objects.get(id=job_id)
        except Jobs.DoesNotExist:
            return Response(
                {"error": "Job no encontrado"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        try:
            ...
            if not (
                ...
                or (
                    ...
                    and request.user.company == job.company
                )
            ):
                return Response(
...
def get_user_embedding_vector(user_id: int) -> list[float]:
    ...
    # user_profile_text = get_user_profile_text(user_id)
    # CV, habilidades, experiencia, etc.
    # payload = {
    ...
    #     f"{settings.EMBEDDING_SERVICE_URL}/text-processing/vectorize/", json=payload
    # )
